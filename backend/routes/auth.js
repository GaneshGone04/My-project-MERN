...
router.post('/register', async (req, res) => {
  ...
  try {
    ...
    const hashedPassword = await bcrypt.hash(password, 10); // here the password is hashed
    const user = new User({ name, email, password:hashedPassword}); // now the user object is created with the hashed-password
    await user.save(); // the User object is saved!
